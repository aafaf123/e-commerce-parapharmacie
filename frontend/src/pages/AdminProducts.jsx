...
import { Plus, ArrowLeft, Edit, Trash2, Search, Save, X, Loader2, Package, ChevronDown, ChevronUp, FileText, Tag, Scan, Filter, Download, Upload, Percent } from 'lucide-react'
...
const AdminProducts = () => {
  ...
  const [lastUsbKeyTime, setLastUsbKeyTime] = useState(0)
    ...
    try {
      ...
      const exportData = allProducts.map(p => ({
        ...
        'Variantes': p.productVariants?.map(v => 
          `${v.type}: ${v.value} - ${v.price || ''}dh - Stock: ${v.stock}`
        ).join(' | ') || ''
      }))
      ...
      if (format === 'pdf') {
        const doc = new jsPDF()
        autoTable(doc, {
          head: [['Nom', 'Catégorie', 'Prix HT', 'Prix TTC', 'Stock', 'Marque', 'Code-barres', 'Actif']],
          body: allProducts.map(p => [
            p.name || '',
            p.category?.name || '',
            p.priceHT?.toFixed(2) || '0',
            (p.price || p.priceTTC)?.toFixed(2) || '0',
            p.stock?.toString() || '0',
            p.brand || '',
            p.barcode || '',
            p.active ? 'Oui' : 'Non'
          ...
          theme: 'grid',
          startY: 20,
          headStyles: { fillColor: [0, 115, 170], textColor: 255 },
          margin: { top: 10 }
        })
        
        doc.text('Catalogue Produits', 14, 15)
        doc.text(`Exporté le: ${new Date().toLocaleDateString('fr-FR')}`, 14, 25)
        doc.text(`Total produits: ${allProducts.length}`, 14, 30)
  ...
  const handleBarcodeLookup = async (barcode) => {
    try {
      ...
      const { data } = await axios.get(`/barcode/lookup/${barcode}`)
      
      // Auto-pre-fill the form
      setFormData(prev => ({
        ...prev,
        ...
        imagePublicId: data.imagePublicId || prev.imagePublicId,
        barcode: barcode
      }))

      // If category looks like something we have, try to match it
      if (data.category && categories.length > 0) {
        // Very basic fuzzy match for category
      ...
      // Automatically open modal if not already open
      if (!isModalOpen) {
        setIsModalOpen(true)
      }
      ...
      setFormError(error.response?.data?.message || 'Produit introuvable via scan')
    } finally {
      setIsLookupLoading(false)
    }
  }

  // USB Scanner Listener (Global)
  useEffect(() => {
    const handleKeyDown = (e) => {
      // Les douchettes USB simulent un clavier ultra-rapide
      const now = Date.now()
      
      // Si le délai entre deux touches est > 50ms, ce n'est probablement pas un scanner
      if (now - lastUsbKeyTime > 50) {
        setUsbBuffer('')
      }
      setLastUsbKeyTime(now)

      if (e.key === 'Enter') {
        if (usbBuffer.length >= 8) { // EAN-8 minimum
          handleBarcodeLookup(usbBuffer)
          setUsbBuffer('')
        }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-3">
              <button
                ...
                className="flex items-center gap-2 bg-green-50 text-green-700 border border-green-200 hover:bg-green-100 px-3 py-2 rounded-lg transition-colors font-medium text-sm"
                disabled={isLookupLoading}
              >
                {isLookupLoading ? <Loader2 size={16} className="animate-spin" /> : <Scan size={16} />}
              ...
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {showAllColumns ? (
                    ...
                    <>
                      {['Image', 'Nom', 'Prix', 'Stock', 'Catégorie'].map(h => (
                    ...
                    {showAllColumns ? (
                      <>
                        <td className="px-2 py-2">
                          {product.image
                            ? <img src={product.image} alt={product.name} className="w-8 h-8 object-cover rounded" onError={e => { e.target.src = '/images/placeholder.jpg' }} />
                      ...
                      <>
                        ...
                        <td className="px-3 py-3 text-sm text-gray-500 max-w-[150px] truncate">{product.category?.name || '—'}</td>
                        <td className="px-3 py-3 text-sm">
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto mx-4 my-6">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              {/* Barcode Field */}
              <div className="bg-green-50/50 p-4 rounded-lg border border-green-100 border-dashed">
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-sm font-semibold text-green-800">Code-barres (EAN)</label>
                  <button 
                    type="button" 
                    onClick={() => setIsScanning(true)}
                    className="text-xs flex items-center gap-1 text-green-700 hover:text-green-900 font-bold"
                  >
                ...
                <p className="text-[10px] text-green-600 mt-1 flex items-center gap-1">
                  <Tag size={10} /> Le scanner USB remplit ce champ automatiquement
                </p>
              </div>
              ...
              <div className="bg-sky-50/50 p-4 rounded-xl border border-sky-100 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                  <div>
                    <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Ancien Prix HT (Optionnel)</label>
                    <input type="number" name="oldPriceHT" value={formData.oldPriceHT} onChange={handleInputChange} step="0.01"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-sky-700 bg-white" />
                  </div>
                  <div className="md:col-span-2">
                    {formData.oldPriceTTC && (
                      <div className="h-full flex items-end pb-2">
                        <span className="text-xs text-gray-500 flex items-center gap-1">
                           Calculé : <span className="line-through">{formData.oldPriceTTC} DH TTC</span>
                        </span>
                      </div>
                    )}
                  </div>
              ...
              <div className="border-t pt-4">
                ...
                {showVariants && (
                  <div className="space-y-4">
                    ...
                    <button
                      type="button"
                      onClick={() => setVariants([...variants, { id: Date.now().toString(), variantTypeId: '', variantTypeName: '', value: '', price: null, stock: 0, image: '', description: '' }])}
      ...
      {isLookupLoading && (
        <div className="fixed inset-0 bg-white/60 backdrop-blur-[2px] flex items-center justify-center z-[60]">
          <div className="bg-white p-6 rounded-2xl shadow-2xl border flex flex-col items-center gap-4">
            <div className="relative">
              <Scan size={40} className="text-sky-700 animate-pulse" />
              <Loader2 size={24} className="absolute -top-1 -right-1 text-sky-500 animate-spin" />
            </div>
            <div className="text-center">
              <p className="font-bold text-gray-900">Analyse du produit...</p>
              <p className="text-sm text-gray-500 italic">Recherche dans la base mondiale</p>
            </div>
          </div>
        </div>
      )}
